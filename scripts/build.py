...
for i, r in enumerate(DATA["github_trending"], 1):
    gh_rows.append(
        ...
        '</div></a>'
        % (esc(r["url"]), i, esc(r["repo"]), esc(r.get("desc", "")),
...
for r in DATA["hackernews"]:
    hn_rows.append(
        ...
        '</a>' % (esc(r["url"]), esc(r["title"]), r.get("points", 0), r.get("comments", 0)))
...
for r in DATA["social"]:
    soc_rows.append(
        ...
        % (esc(r["url"]), esc(r["handle"]), esc(r["title"]), esc(r.get("note", ""))))
...
.nav-links{display:flex;gap:26px;font-size:14px;color:var(--fg-dim)}
...
.hero p{font-size:clamp(1rem,2.2vw,1.25rem);color:var(--fg-dim);max-width:560px}
...
.repo-desc{font-size:13.5px;color:var(--fg-dim);margin:4px 0 9px}
...
.post-title{font-weight:500;font-size:14.5px;transition:color .2s}
.post-meta{display:flex;gap:14px;font-size:12px;color:var(--fg-dim);margin-top:7px}
...
.social-title{font-weight:500;margin:10px 0 8px;line-height:1.35}
.social-note{font-size:13px;color:var(--fg-dim)}
.rd-sub{color:#FB923C;font-weight:600;font-family:'Space Grotesk';font-size:13px}
.rd-title{font-weight:500;margin:9px 0 10px;line-height:1.35;font-size:14.5px}
...
<meta name="description" content="매일 업데이트되는 AI 트렌드 — GitHub Trending, Hacker News, Reddit, 화제의 SNS 한눈에.">
...
<header><div class="wrap nav">
  ...
  <nav class="nav-links"><a href="#github">GitHub Trending</a><a href="#hn">Hacker News</a><a href="#social">화제의 SNS</a></nav>
</div></header>
<main class="wrap">
  <section class="hero">
    <span class="badge"><span class="live"></span>매일 자동 업데이트 · __DATE__</span>
    <h1>오늘의 <span class="g">AI 트렌드</span><br>한 화면에서.</h1>
    <p>GitHub에서 가장 뜨거운 저장소, Hacker News의 핵심 논의, Reddit과 화제가 된 SNS 글을 매일 자동으로 모아 정리합니다.</p>
...
<footer><div class="wrap">
  <div>마지막 업데이트: __DATE__ · 데이터: GitHub Trending, Hacker News, Reddit, X</div>
...
for k, v in [("__CSS__", CSS), ("__DATE__", esc(DATA["date_label"])),
             ...
             ("__TOPSTARS__", "{:,}".format(top_stars)),
             ("__GH__", "".join(gh_rows)), ("__HN__", "".join(hn_rows)),
             ("__REDDIT__", reddit_card), ("__SOCIAL__", "".join(soc_rows))]:
